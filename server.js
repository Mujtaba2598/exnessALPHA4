...
const JWT_SECRET = process.env.JWT_SECRET || 'halal-exness-secret-key';
...
class TradingEngine {
    constructor(sessionId, userEmail, config, bridgeConnected) {
    ...
    async start() {
        ...
        const runAnalysis = async () => {
            ...
            // Continue analysis every 2 seconds (not too aggressive to avoid rate limits)
            if (this.isActive) {
            ...
            for (const [symbol, position] of this.openPositions) {
                try {
                    ...
                    if (position.side === 'buy') {
                        unrealizedProfit = (priceData.bid - position.entryPrice) * position.volume * 100000;
                    } else {
                        unrealizedProfit = (position.entryPrice - priceData.ask) * position.volume * 100000;
                    }
                    
                    const profitPercent = (unrealizedProfit / position.positionValue) * 100;
                    
                    // Halal profit taking - no interest, just market spread
                    if (profitPercent >= this.config.takeProfit) {
                        console.log(`📈 TAKING HALAL PROFIT: ${symbol} at ${profitPercent.toFixed(2)}%`);
                        await this.closePosition(symbol, position);
                    } else if (profitPercent <= this.config.stopLoss) {
                        console.log(`📉 STOP LOSS: ${symbol} at ${profitPercent.toFixed(2)}%`);
                        await this.closePosition(symbol, position);
                    }
                } catch (error) {
    ...
    async executeTrade(symbol, side, signal) {
        ...
        try {
            ...
            let volume = this.config.investmentAmount / 100000; // Convert to lot size
            ...
            console.log(`📈 EXECUTING HALAL ${side.toUpperCase()}: ${symbol} with ${volume} lots at $${entryPrice}`);
            
            const result = await sendToBridge('placeOrder', {
                symbol: symbol,
                volume: volume,
                side: side,
                sl: side === 'buy' ? entryPrice * (1 - Math.abs(this.config.stopLoss)/100) : entryPrice * (1 + Math.abs(this.config.stopLoss)/100),
                tp: side === 'buy' ? entryPrice * (1 + this.config.takeProfit/100) : entryPrice * (1 - this.config.takeProfit/100)
            });
            ...
            console.log(`✅ Halal ${side.toUpperCase()} opened for ${symbol}`);
        } catch (error) {
    ...
    async closePosition(symbol, position) {
        try {
            const result = await sendToBridge('closeOrder', { orderId: position.orderId });
            const priceData = await sendToBridge('getPrice', { symbol });
            const exitPrice = position.side === 'buy' ? priceData.bid : priceData.ask;
            
            let profit = 0;
            if (position.side === 'buy') {
                profit = (exitPrice - position.entryPrice) * position.volume * 100000;
            } else {
                profit = (position.entryPrice - exitPrice) * position.volume * 100000;
            }
            ...
            this.trades.unshift({
                ...
                profitPercent: ((profit / position.positionValue) * 100).toFixed(2),
                timestamp: new Date().toISOString()
            });
            ...
            console.log(`✅ CLOSED ${symbol} | Profit: ${profitSymbol}$${profit.toFixed(2)} (${((profit / position.positionValue) * 100).toFixed(2)}%) | Total: $${this.currentProfit.toFixed(2)}`);
        } catch (error) {
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        const { investmentAmount, targetProfit, timeLimit, tradingPairs, takeProfit, stopLoss } = req.body;
        ...
        if (!testResult || testResult.balance < investmentAmount) {
            return res.status(400).json({ success: false, message: `Insufficient balance. You have $${testResult?.balance?.toFixed(2) || 0} USDT, need $${investmentAmount}` });
        }
        ...
        const engine = new TradingEngine(sessionId, req.user.email, config, true);
        engines[sessionId] = engine;
        await engine.start();
        
        console.log(`✅ Halal trading started for ${req.user.email} | Investment: $${investmentAmount} → Target: $${targetProfit}`);
        res.json({ 
            success: true, 
            sessionId, 
            message: `✅ HALAL TRADING STARTED! Balance: $${testResult.balance.toFixed(2)} | Investment: $${investmentAmount} | AI analyzes continuously | Take profit: ${config.takeProfit}% | Stop loss: ${config.stopLoss}% | 100% Halal` 
        });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Positions close at profit target or stop loss`);
    console.log(`✅ 100% Halal - No interest, Spot/Forex trading only\n`);
});
